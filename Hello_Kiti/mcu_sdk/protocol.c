 ...
 */
extern	enum feed_state f;
void all_data_update(void)
{
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"

	extern	uint8_t quick_feed;
	extern	uint8_t manual_feed;
	extern	uint8_t light;
    //此代码为平台自动生成，请按照实际数据修改每个可下发可上报函数和只上报函数
    mcu_dp_bool_update(DPID_QUICK_FEED,quick_feed); //BOOL型数据上报;
    mcu_dp_value_update(DPID_MANUAL_FEED,manual_feed); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_FEED_STATE,f); //枚举型数据上报;
    mcu_dp_bool_update(DPID_LIGHT,light); //BOOL型数据上报;
